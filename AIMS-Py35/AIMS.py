...
class Likelihood:
            ...
            if (utilities.is_number(columns[0])):
                ...
                if (utilities.is_number(columns[1])):
                    like.add_constraint((columns[0],           \
                        ...
                        list(map(utilities.to_float,columns[1:])))))
                else:
                    like.add_constraint((columns[0],           \
                        Distribution(columns[1],               \
                        list(map(utilities.to_float,columns[2:])))))
    ...
    def add_constraint(self,name_distribution):
        """
        ...
        # tuple unpacking outside of function definition:
    ...
    def find_map(self, my_model, use_n):
        ...
        if (use_n): # No, this is not a mistake - you wan't to use the local value of use_n, not the
            ...
            for i in range(len(self.modes)):
                found = False
                for j in range(len(my_model.modes['n'])):
                    if self.nvalues[i] == my_model.modes['n'][j] and self.lvalues[i] == my_model.modes['l'][j]:
                        mode_map[i]=j
                        found = True
                        break
                if not found:
...
class Probability:
    ...
    def evaluate(self, my_model):
        ...
        if (config.surface_option is None):
            ...
            result2 = self.priors(list(map(my_model.string_to_param,grid_params_MCMC)))
        else:
            result1, surf_amplitudes = self.likelihood.evaluate(my_model)
            result2 = self.priors(list(map(my_model.string_to_param,grid_params_MCMC))+list(surf_amplitudes))
...
def find_best_model():
    ...
    best_grid_params  = list(map(best_grid_model.string_to_param,grid_params_MCMC))
    ...
    best_grid_params = best_grid_params + list(map(best_grid_model.string_to_param,config.output_params))
...
def find_best_model_in_track(ntrack):
    ...
    for model in grid.tracks[ntrack].models:
        ...
        if (result >= threshold):
            accepted_parameters_local.append(list(map(model.string_to_param,grid_params_MCMC)))
        else:
            rejected_parameters_local.append(list(map(model.string_to_param,grid_params_MCMC)))
...
def find_a_blob(params):
    ...
    my_model = model.interpolate_model(grid,params[0:ndims-nsurf],grid.tessellation,grid.ndx)
    return list(map(my_model.string_to_param,config.output_params))
    ...
    if (config.tight_ball):
        for name in grid_params_MCMC_with_surf:
            output_file.write(str_string.format("Tight ball range on "+name, \
                str(config.tight_ball_range[name])))
...
def interpolation_tests(filename):
    ...
    titles = list(map(model.string_to_latex,grid.grid_params+("Age",)))
...
if __name__ == "__main__":
    ...
    list(map(like.add_seismic_constraint,config.seismic_constraints))
    ...
    # titles, labels, etc.
    labels     = ["ln(P)"]+list(map(model.string_to_latex,grid_params_MCMC_with_surf))
    labels_big = labels + list(map(model.string_to_latex,config.output_params))
    ...
    best_MCMC_params = list(best_MCMC_params) + list(map(best_MCMC_model.string_to_param,config.output_params))
    ...
    if (statistical_model is not None):
        statistical_params = list(statistical_params) + list(map(statistical_model.string_to_param,config.output_params))
