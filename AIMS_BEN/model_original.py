...
def string_to_latex(string,prefix="",postfix=""):
    ...
    if (string == "nu0_15"):     return r'$%s\nu_{15,0}%s$ (in $\mu$Hz)'%(prefix,postfix)
...
class Model:
    ...
    def string_to_param(self,string):
        ...
        if (string == "nu0_15"):     return self.glb[ifreq_ref]*self.find_mode(15,0)
    ...
    def write_file_simple(self,filename):
        ...
        .. note::
          - Frequencies are dimensional and should be expressed in muHz
        """
...
class Track:
    ...
    def __init__(self, aModel, grid_params):
        ...
        self.nmodes = aModel.modes.shape[0]
    ...
    def thin(self, n):
        ...
        :param n: thinning factor
        :type n: int
        """

        self.models = self.models[0::n]
        self.nmodes = sum([model.modes.shape[0] for model in self.models])
    ...
    def read_model_list(self,filename):
        ...
        for track in self.tracks:
            ...
            if duplicate[0]:
                ...
                print("       Check models: %s %s"%(str(duplicate[1]),str(duplicate[2])))
                sys.exit(1)
...
def interpolate_model(grid,pt,tessellation,ndx):
    ...
    if (n == 1):
        if (abs(coefs[0]-1.0) > eps):
            print("WARNING: erroneous interpolation coefficient: %f"%(coefs[0]))
