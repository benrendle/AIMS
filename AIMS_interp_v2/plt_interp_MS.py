...
import plot_track_freqs as ptf
...
def plot_grid(grid, dim, ax_titles, title = None):
    """
    ...
    plt.xlim((xmin,xmax))
    plt.ylim((ymin,ymax))
    plt.xlabel(ax_titles[0],fontsize=12)
    plt.ylabel(ax_titles[1],fontsize=12)
...
def plot_slice_age(pos):
    ...
    if (all_nan(results_age[0][pos][:,ndim:ndim+3]) and \
        ...
        plt.title(r"Error at $(M,Z)=(%f,%f)$"%(results_age[0][pos][0,0],results_age[0][pos][0,1]),fontsize=16)
        plt.ylabel(r"Error(radial)",fontsize=12)
        plt.yscale('log')
        plt.xlim(xlim)
        plt.tick_params(labelsize=12)
        ...
        for j in xrange(2):
            for i in xrange(3):
                plt.plot(results_age[j][pos][:,ndim-1],results_age[j][pos][:,ndim+3+i],
                     style[i+3*j],label=labels[i+3*j])
        plt.ylabel(r"Error(non radial)",fontsize=12)
        plt.yscale('log')
        plt.xlabel(titles[ndim-1],fontsize=12)
        plt.xlim(xlim)
        plt.tick_params(labelsize=12)
        plt.legend(fontsize=12)	##################################################################################################################
    plt.show()
...
def plot_partition_tessellation(grid, ndx1, ndx2, tessellation):
    ...
    plt.xlim((xmin,xmax))
    plt.ylim((ymin,ymax))
    plt.xlabel(titles[0],fontsize=12)
    plt.ylabel(titles[1],fontsize=12)
    plt.tick_params(labelsize=12)
...
def plot_slice_track(pos):
    ...
    if (all_nan(results_track[pos][:,ndim:ndim+3])):
        ...
        plt.title(r"Error at $(M,Z)=(%f,%f)$"%(results_track[pos][0,0],results_track[pos][0,1]),fontsize=16)
        plt.ylabel(r"Error(radial)",fontsize=12)
        plt.yscale('log')
        plt.xlim(xlim)
        plt.tick_params(labelsize=12)
...
def surface2D(p,results,error_ndx, dim, tpe="max",title=None,truncate=0, ax_titles=titles, params=(0, 1)):
    """
    ...
    :param truncate: (default = 0): specifies how many models should be omitted
      on both ends of the track.  This is useful for comparing results from
      tests involing different sizes of increments.
    :param ax_titles: titles of the axes
    ...
    :type title: string
    :type truncate: int
    :type ax_titles: list of strings
    ...
    for i in xrange(n):
        ...
        stop  = results[i].shape[0] - truncate
        if (tpe == "max"):
            value = np.nanmax(results[i][start:stop,dim+error_ndx])
        elif (tpe == "avg"):
            num = den = 0.0
            for j in xrange(start,stop):
                if (not math.isnan(results[i][j,dim+error_ndx])):
                    num += results[i][j,dim+error_ndx]**2
                    den += 1.0
            if (den > 0.0):
                value = math.sqrt(num/den)
            else:
	    ...
	    z2.append(math.log10(value))
            x.append(results[i][0,params[0]])
            y.append(results[i][0,params[1]])
...
#        pass
    ...
    cax.text(4.2,0.7,r"Percentage Error",rotation=270,fontsize=12)	# %s %(tpe)
...
if __name__ == "__main__":
    ...
    assert (len(sys.argv) > 1), "Usage: plot_interpolation_test.py interp_file grid_file"
    ...
    surface2D(1,results_age1,0,ndim,tpe="avg",title="Avg radial error (nincr = 1)",truncate=1, ax_titles=titles)
    ...
    plot_grid(grid, ndim, titles)
    #plot_partition_tessellation(grid, ndx1, ndx2, tessellation)

    # The following allows for plotting of best and worst tracks in the model grid by integrating
    # plot_track_freqs.py into the interpolation plotting program. This requires the original grid
    # to be passed as an additional argument. Only plots of the best and worst tracks in the grid
    # will be plotted so to obtain the 'n' best and 'n' worst tracks, run plot_track_freqs individually.

    if len(sys.argv) == 3:
        grid_name = sys.argv[2]
        grid_file = open(grid_name, "r")
        grid_data = dill.load(grid_file)
        grid_file.close()

        choose_tracks = ptf.find_high_low_deviation(grid_data, 1)
        worst_id = choose_tracks[0]
        worst_variance = choose_tracks[1]
        best_id = choose_tracks[2]
        best_variance = choose_tracks[3]
        print "Models with highest variance: %d Variance: %f" % (worst_id[0], worst_variance[0])
        print "Models with lowest variance: %d Variance: %f" % (best_id[0], best_variance[0])
        ptf.plot_tracks(grid_data, np.append(worst_id, best_id), 3)
